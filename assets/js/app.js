...
(function () {
  ...
  function loadFavorites() {
    try {
      return new Set(JSON.parse(localStorage.getItem('hm_favorites') || '[]'));
    } catch (e) { return new Set(); }
  }
  function saveFavorites() {
    localStorage.setItem('hm_favorites', JSON.stringify(Array.from(favorites)));
  }

  // 마지막으로 사용한 지역·빠른필터 버튼 상태를 이 기기에 기억한다
  function loadLastFilters() {
    try {
      return JSON.parse(localStorage.getItem('hm_lastFilters')) || null;
    } catch (e) { return null; }
  }
  function saveLastFilters() {
    try {
      ...
      localStorage.setItem('hm_lastFilters', JSON.stringify(toggles));
    } catch (e) {}
  }
  ...
  function matches(f) {
    ...
    if (state.kidsCare && !f.kidsCare) return false;
    if (state.erCare && !f.erCare) return false;
    if (state.favOnly && !favorites.has(f.id)) return false;
    if (state.q) {
      var q = state.q.toLowerCase();
      var hay = [f.name, f.address, f.district, f.region, f.kind].join(' ').toLowerCase();
  ...
  function locateOnMap(id) {
    var f = HOSPITALS.find(function (x) { return x.id === id; });
    if (!f) return;
  ...
  /* ---------- 렌더 파이프라인 ---------- */
  function render() {
    currentList = nearby.sort(HOSPITALS.filter(matches));
  ...
  function buildFilterPills() {
    ...
    var presentR = {};
    HOSPITALS.forEach(function (f) { presentR[f.region] = true; });
    ...
    var presentK = {};
    HOSPITALS.forEach(function (f) { presentK[f.kind] = true; });
  ...
  function setDistrict(key) {
    ...
    if (key) {
      var parts = key.split('|');
      var sub = HOSPITALS.filter(function (f) { return f.region === parts[0] && f.district === parts[1]; });
      if (sub.length) {
  ...
  document.getElementById('resetBtn').addEventListener('click', function () {
    ...
    state.kidsCare = false; state.erCare = false; state.favOnly = false;
